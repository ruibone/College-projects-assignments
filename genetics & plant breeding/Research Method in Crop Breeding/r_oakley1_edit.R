...
wh <- which(cg > 0.999, arr=TRUE) # Feel free to vary the number and make your own decision in the final report
wh <- wh[wh[,1] < wh[,2],] # remove duplicated information
wh #要刪掉4,5,6,9,27,46,89,158,234,282,326,345,354,384,392,399,441,478,479
...
## Group markers
##========================================================##
lg <- formLinkageGroups(crox, max.rf=0.3, min.lod=6)
table(lg[,2])
...
#首先，其實在真實已知的情況下只要跑夠多次是可以解決問題的
system.time(croa <- orderMarkers(cro1, chr = 1, use.ripple = TRUE, window = 8, map.function = "kosambi"))
#window先調到8，結果沒變
#要跑很久 4617.33 

xo <- countXO(crox) #計算crossover過高的個體
hist(xo, breaks=50)
crox <- subset(crox, ind=(xo < 14 & xo > 1)) #刪掉大於14的個體

crox <- calc.errorlod(crox, err=0.05) #計算可能genotype錯誤的LOD值
top.errorlod(crox) #發現有2個marker在第一條染色體上
croa<-drop.markers(croa, c("X1_5876257","X1_20668556")) #刪掉marker
                   
markernames(croa, chr = 1) #看名字裡有位置順序
croa <- switch.order(croa,1,c(11:1,12:48,88:49)) # 手動換(因為這條LG裡似乎又細分成3個LG)

# tryallpositons在此不適用，會有多個marker在同個位置有高LOD

croa <- est.map(croa) #重新計算marker間的距離


markernames(cro1,chr = 1)
crob <- switch.order(cro1,1,c(11:1,12:49,90:50)) #不刪掉marker也能完成
